...
namespace MiniTaller.Repositorios.Repositorios
{
    public class RepositorioDeVehiculos:IRepositorioDeVehiculos
    {
        ...
        public List<VehiculosComboDto> GetVehiculosCombos()
        {
            ...
            using (var conn = new SqlConnection(cadenaDeConexion))
            {
                string selectQuery = @"SELECT v.IdVehiculo, CONCAT(UPPER(v.Patente),' | Tipo Vehiculo: ',t.TipoVehiculo,' | Modelo: ', m.Modelo) AS Info FROM Vehiculos v
...
// NO TE OLVIDES QUE TANTO LAS ENTIDADES Y LAS PROPIEDADES DE CADA ENTIDAD DEBEN TENER LOS MISMOS NOMBRES, PARA QUE EL QUERY FUNCIONE
// SINO PUEDE QUE NO TE TRAIGA DATOS SOMPLEMENTE POR NO LLAMARSE IGUAL
